...
<!-- why: se agregó el 2026-08-21 al aprobar generar-instrumento-impreso como skill nueva,
     distinta de generar-clase-impresa (esa resume/re-narra para estudiar; esta transcribe
     fiel un instrumento con nota o práctica para que se rinda en papel sin computador)
     evita: que se confunda con generar-clase-impresa y se termine resumiendo o
     re-narrando una evaluación/control/ejercitación en vez de transcribirla completa
     ej: ✅ activarla cuando Diego pide "pasar a papel" una evaluación para evitar copia
         ❌ usar generar-clase-impresa para una Evaluación o Control -->
- **Instrumento impreso antitrampa (opcional, on-demand)**: si Diego quiere que una Evaluación, Ejercitación/Simulacro o Control se rinda en papel sin computador (evitar copia), activa `generar-instrumento-impreso`. Ver "Workflow opcional: instrumento impreso antitrampa" más abajo.
...
## Workflow opcional: instrumento impreso antitrampa (Evaluación/Ejercitación/Control Impresa)

Workflow paralelo al flujo maestro, no una etapa más — se activa on-demand cuando Diego quiere que una Evaluación, Ejercitación/Simulacro o Control se rinda **en papel, sin computador**, para evitar que los estudiantes copien o busquen respuestas en internet. Lo pedirá con frases como "pasa a papel la evaluación X", "necesito que no puedan hacer trampa", o "genera la versión impresa del control/ejercitación".

**Requisito previo:** el notebook de estudiante del instrumento (`Evaluación.ipynb`, `Ejercitación.ipynb` o `Control.ipynb`) ya aprobado.

**Qué produce:** `Clase NN - Tema - [Evaluación|Ejercitación|Control] Impresa.tex`/`.pdf` — a diferencia de la Clase Impresa de arriba, **sí es una transcripción fiel y completa** del instrumento: mismos ítems, mismas narrativas, mismos códigos, mismas pistas, mismos ejemplos — nada se resume ni se re-narra, porque el objetivo es medir exactamente lo mismo que el instrumento digital, solo que a mano.

Activa la skill `generar-instrumento-impreso` — ahí está el sistema de diseño completo (motor LaTeX/tectonic, cajas siempre con fondo blanco, encabezado compacto sin subtítulo, regla de indentación con líneas guía, un ejercicio de desarrollo por plana). `clases/clase-27-evaluacion-ciclos/Clase 27 - Evaluación Ciclos - Evaluación Impresa.tex` y `clases/clase-26-simulacro-ciclos/Clase 26 - Simulacro Ciclos - Ejercitación Impresa.tex` son las plantillas de referencia (primer caso aprobado, 2026-08-21).

**Publicación:** un instrumento con nota (Evaluación/Control) impreso no se pushea hasta después de rendido — mismo criterio que el resto de artefactos de esa evaluación/control.
...
5. **Ninguna solución vive en el notebook de estudiante, ni siquiera oculta con `<details>`.** Ni `Clase.ipynb` ni `Ejercicios.ipynb` contienen soluciones en ninguna forma — todas (Ticket de Salida, Práctica Independiente, Ejercicios) viven exclusivamente en `Solucionario.ipynb`, que Diego sube a Classroom recién después de dictar la clase. Esto evita que cualquier estudiante que abra el notebook del curso vea una respuesta antes de tiempo, y le permite a Diego usar el mismo archivo para revisar en vivo cuando trae a una pareja a explicar al frente.
   ...
   > **Excepción única — el Colab de Ejercitación de los lunes estándar (desde 2026-08-12).** El notebook de ejercitación de la Fase 1 de un lunes estándar (ver "Workflow: lunes estándar") **sí** cierra con una sección `## 🔓 Soluciones` en la que cada solución va colapsada dentro de un `<details>`. Aplica solo a ese archivo: el Colab del Control de la Fase 2 sigue sin ninguna solución, y las clases de martes/jueves siguen con la regla general sin cambios. La razón por la que aquí no hay filtración que evitar: lo que se mide con nota ese día es el Control, no la ejercitación — la ejercitación es entrenamiento, y poder verificar la propia respuesta apenas se termina un ejercicio es justamente lo que se busca. Contraparte obligatoria: los ítems del Control nunca son los mismos ejercicios con otros números (ver "Workflow: lunes estándar").
...
15. **Formato canónico de ejercicios de Práctica Independiente — el mismo que usa la Práctica Guiada (regla 20), default desde Clase 20.** Cada ejercicio (y la Guiada) sigue esta estructura fija en orden:
    ...
    3. Pistas colapsables con `<details><summary>💡 Pista N — subtítulo</summary>...</details>` — **default siempre desde Clase 24b (2026-08-19), 1-2 por ejercicio a criterio de quien diseña** (antes: solo donde aplica). Cada ejercicio de Práctica Independiente lleva al menos una pista; el criterio de cuántas (1 o 2) y sobre qué punto (un operador que no es el foco de la clase, un paso del patrón general que se presta a confusión, un caso borde) queda a discreción del diseño, no de si "amerita" o no. **Alcance ampliado desde Clase 25 (2026-08-19): este default "siempre 1-2 pistas" aplica a cualquier serie de ejercicios de práctica sin nota — incluida la `Ejercitación.ipynb` de un lunes estándar y los ejercicios de una ayudantía — no solo a la Práctica Independiente de una clase regular.** Única excepción permanente: instrumentos con nota (`Control.ipynb`, `Evaluación.ipynb`) — ahí el default es sin pistas, y Diego indica explícitamente, instancia por instancia, si quiere agregarlas a un ítem en particular; no asumir ninguna de las dos opciones sin que él lo diga para ese Control/Evaluación específico.
...
**Presupuesto de palabras (default desde Clase 20 v2, 2026-08-05).** Para las clases regulares de martes/jueves, la narrativa + bullets de cada pieza (Guiada y cada ejercicio de Independiente) apunta a **60-90 palabras**, sin contar el bloque de resultado esperado — así los tres ejercicios obligatorios (regla 16) alcanzan a resolverse dentro del tiempo asignado a la sección. Este presupuesto es propio de las clases: las ayudantías de los lunes (`disenar-ayudantia-ejercicios`) siguen su propio formato más extenso, pensado para consolidar sin presión de tiempo, no para este límite. Si un enunciado no cabe, prioriza recortar bullets de instrumentación redundante (ver "Verificador por salida" en `generar-colab-clase/SKILL.md`) antes que la narrativa — sin narrativa el ejercicio se vuelve "solo ejecutar", justo lo que este formato busca evitar. El Ejercicio 3 (contextualizado, regla 16) puede acercarse con naturalidad al extremo superior del rango, y el ejercicio de desafío (Ejercicio 4) puede exceder algo este presupuesto, ya que en ambos se busca una narrativa más rica.
16. **Práctica Independiente: 3 ejercicios obligatorios (2 directos + 1 contextualizado y más complejo) + 1 ejercicio de desafío opcional (default desde Clase 24, 2026-08-12 — reemplaza el esquema anterior de 2 obligatorios + desafío, vigente desde Clase 20 v2).** No preguntar la cantidad — es fija. Ejercicios 1 y 2 aplican directo el concepto principal de la clase: mismo formato canónico y misma exigencia de narrativa (regla 15) que el resto; ninguno se presenta como bonus ni con narrativa más breve. El Ejercicio 3 sube un escalón: narrativa más contextualizada, combina o profundiza el concepto (puede mezclarlo con uno secundario ya visto) — es también el nivel que fija el techo de dificultad de la Práctica Guiada (regla 20). El Ejercicio 4 (desafío) es opcional, para quien termine antes los tres obligatorios: mismo formato canónico, mismo escenario que el resto de la Independiente cuando aplique, con margen para una narrativa algo más rica (ver nota de presupuesto de palabras arriba). Diego sigue revisando en vivo solo uno de los obligatorios (su elección), trayendo a una pareja a explicarlo al curso (regla 18).

    <!-- why: piloto probado en la Ejercitación de Clase 25 (2026-08-21) — Diego pidió
         ejercicios de calentamiento que apliquen la definición pelada del contenido
         (tipo "define una función que haga X"), más directos que los Ejercicios 1-2
         actuales, que ya traen narrativa aunque sean "directos". Confirmó que le gustó
         y pidió mantenerlo de ahora en adelante para clases de contenido, explícitamente
         no para Simulacro ni para pruebas (Control/Evaluación)
         evita: que se agreguen ejercicios "0" a instrumentos con nota, o que se le
         pregunte a Diego la cantidad en vez de decidirla según cuántos matices
         distintos tiene el contenido de la clase
         ej: ✅ Ejercicio 0a (definir+return+imprimir) y 0b (reutilizar el resultado
             guardado) antes del Ejercicio 1 de una Ejercitación o Práctica Independiente
             ❌ agregar un Ejercicio 0 a Control.ipynb, Evaluación.ipynb o un Simulacro -->
    **Ejercicios "0" de práctica directa — prefijo opcional antes del Ejercicio 1, default desde 2026-08-21.** 1-2 ejercicios `Ejercicio 0a` / `Ejercicio 0b` que aplican la definición pelada del contenido, sin narrativa, sin disfrazar el escenario y sin pistas — consigna técnica explícita del estilo "Define una función/un ciclo que haga X, evalúa el resultado guardándolo en una variable e imprímelo" (única excepción a la regla 8 de no nombrar el constructo, porque el propósito es justamente aplicar la definición pelada). Cantidad a criterio de quien diseña, igual que las pistas (regla 15.3): 2 cuando la clase tiene dos matices claramente distintos que ameritan drill separado (ej. `return` vs `print()`, y reutilizar un resultado guardado), 1 cuando hay un solo patrón central. Aplica a la Práctica Independiente de clases regulares (esta regla) y a la `Ejercitación.ipynb` de un lunes estándar (ver esa sección más abajo). **Nunca a Simulacro (`generar-simulacro-evaluacion`) ni a Evaluación/Control** (instrumentos con nota): ahí el objetivo es medir en el mismo formato del instrumento real, no calentar con la definición pelada — Diego no quiere que se pregunten estos ejercicios "0" en ningún Control.
17. **Ticket de Salida: 3 preguntas de alternativas fijas (4 opciones, rotuladas A/B/C/D), siempre — default desde Clase 20 (reemplaza la regla anterior de 2 preguntas si el ICN tenía 1-2 conceptos, 3 si tenía 3+).** No preguntar la cantidad. Las preguntas nunca van en el Colab de estudiante, solo un aviso de que se proyectan en la tele. Se responden vía un Google Form de registro recurrente (`https://forms.gle/sjRpbgmQzrpkEBsH9`). Dinámica: Diego proyecta las preguntas una por una sin revelar nada; cada estudiante decide su respuesta en silencio; recién al terminar la última pregunta completan y envían el Form (Nombre + "Tema de la clase de hoy" + las alternativas marcadas); luego Diego revisa en conjunto las respuestas correctas. Razón del orden (form antes de revelar): evita el efecto arrastre — cada estudiante se compromete con su propia respuesta antes de ver la de los demás. El Colab de clase (`Clase.ipynb`) incluye una sección `## 🎫 Ticket de Salida` justo antes del Cierre con ese aviso + el link del Form + el nombre breve a escribir en "Tema de la clase de hoy" (el generador lo arma solo, sin nada que agregar en el spec) — nunca las preguntas ni alternativas. Las preguntas completas viven en `Clase NN - Tema - Solucionario.ipynb` (enunciado + 4 alternativas rotuladas A/B/C/D + respuesta correcta + justificación breve), en su propia sección separada de las soluciones de Práctica Independiente y Ejercicios. **Cada pregunta incluye un bloque de código breve** aludiendo a la pregunta (default desde Clase 20) — alternando entre código como foco (se predice output o comportamiento) y código como referencia (solo ilustra el enunciado conceptual). **Prioriza predicción de salida directa ("¿qué imprime este programa?") sobre preguntas "truco" centradas en un detalle puntual** (ej. "¿qué pasa si elimino esta línea marcada?", descartado en Clase 20 v2 por "muy particular") — las alternativas deben representar errores de comprensión reales (orden invertido, ciclo que no se repite, valor fijo en vez de dependiente de otra variable), no solo variaciones cosméticas del código. Y antes de reusar el código de un ejemplo del ICN para una pregunta, confirma que sea contenido que Diego realmente va a enseñar/enfatizar — que un concepto aparezca como ejemplo en el ICN no lo vuelve automáticamente evaluable (default desde Clase 20 v2, 2026-08-05). Si la pregunta pide identificar una línea específica, márcala con un comentario inline (ej. `print()  # <- esta línea`) para no generar ambigüedad al proyectarla brevemente. **Las respuestas correctas se reparten en letras distintas entre las preguntas de un mismo Ticket** — nunca todas caen en la misma alternativa (ej. todas B). Además, cada clase con Ticket de Salida genera `Clase NN - Tema - Ticket de Salida Respuestas.json` — solo las respuestas correctas, con llaves que replican tal cual las columnas que el Form genera en la Sheet ("Respuestas a ticket [1]".."[4]", "No se preguntó" en las que sobran si hubo menos de 4 preguntas), pensado para que el agente que cruce las respuestas del Form con la nómina no tenga que leer el Solucionario completo. **El mismo Form incluye, desde el 2026-08-04, una 5ta pregunta fija de autoevaluación** ("Del 1 al 5, donde 1 es 'no entendí' y 5 es 'entendí todo', ¿cuánto entendiste el objetivo de hoy?") — a diferencia de las 3 preguntas MCQ, esta **no se proyecta** (no tiene respuesta correcta que ocultar; el estudiante la responde directo en el Form, junto con el resto, sin verla en ninguna slide) y **no tiene respuesta correcta que registrar**, así que no forma parte de `Ticket de Salida Respuestas.json` ni del Solucionario — solo alimenta `clases/Ticket de Salida - Trazabilidad.xlsx` (ver skill `trazabilidad-ticket-salida`), con desglose por clase y por estudiante. **Las preguntas que se proyecten en la TV van en un PPT aparte de la Presentación principal** (`Clase NN - Tema - Ticket de Salida.pptx`), con el mismo diseño/plantilla que `Presentación.pptx` pero como archivo independiente — nunca como slides agregadas al PPT principal. Razón: `Presentación.pptx` se sube a Classroom/Colab antes de dictar la clase, así que cualquier pregunta que viva ahí queda expuesta a los estudiantes con anticipación. **El PPT del Ticket de Salida se genera junto con el Colab de clase (vigente desde 2026-08-13 — antes se generaba recién el día de la clase).** `generar-colab-clase` lo produce en el mismo paso que `Clase.ipynb` + `Solucionario.ipynb` + el JSON de respuestas, así Diego lo revisa junto con el resto del gate. Lo que sigue reservado para el día de la clase, después de dictarla, es solo su **commit/push** — el repo es público, así que este archivo nunca se sube a GitHub antes de proyectarlo (ver `generar-colab-clase/SKILL.md` § "Después de generar").
...
20. **Práctica Guiada: mismo formato canónico que Práctica Independiente (regla 15), default desde Clase 20.** Comparte escenario con el Haz Ahora (regla 9) y se redacta como un solo ejercicio guiado: narrativa (sin etiqueta "Situación") + `**El programa debe:**` + pistas opcionales + resultado esperado — nunca celda "Mis respuestas" (Diego siempre escribe el código directamente en `# Tu programa`). En el spec, esto reemplaza a `**Pasos guiados (tabla):**` como formato por defecto. **Nivel de dificultad: igual al Ejercicio 3 de la Independiente, no a los ejercicios 1-2 directos (default desde Clase 24, 2026-08-12).** La Guiada es el puente hacia la exigencia contextualizada del Ejercicio 3 — y, en las clases que alimentan un lunes estándar, hacia el ítem de cierre del Control (ver "Workflow: lunes estándar"). **Formato anterior (retrocompatible, solo para regenerar clases previas a Clase 20):** tabla de 2 columnas (`Qué debe hacer tu programa` | `Resultado esperado`) a partir de `**Pasos guiados (tabla):**` con un bloque `- Paso N: ... / Resultado: \`\`\`...\`\`\`` por fila. Formato exacto de ambos y el criterio con que el parser detecta cuál usa cada spec, en `generar-colab-clase/SKILL.md`.
...
<!-- why: la Idea clave de cada concepto del ICN se perdía como bullet plano al final de la definición, sin destaque real ni disciplina de largo — Diego comparó filosofías de redacción y diseños visuales en un notebook de prueba (clases/_pruebas-diseno/) el 2026-08-21 y eligió esta combinación
     evita: una Idea clave que sea un resumen genérico del concepto, muy larga para anotar en clase, o visualmente indistinguible del resto del texto
...
25. **ICN con formato "Escríbelo tú", siempre (default desde 2026-09-24; piloto Clase 33, 2026-09-08, generalizado al corregir Clase 35 — antes había quedado como "demo, NO default" y nunca se activó porque `disenar-clase` no pedía los campos).** Cada concepto del ICN trae, además de Definición/Ejemplo/Idea clave/Resumen tabla, dos campos: `Instrucción` (una frase imperativa que nombra la sintaxis nueva a escribir — única excepción a la regla 8 de no nombrar el constructo) y `Punto de partida` (bloque de código con solo lo que ya existía antes de esa sintaxis, sin la línea que la resuelve). La celda de código del estudiante en `Clase.ipynb` trae el "Punto de partida" incompleto, con la línea `✍️ **Escríbelo tú:** <instrucción>` debajo de la Idea clave — el estudiante escribe él mismo la sintaxis nueva, en vez de solo ejecutar un ejemplo ya completo. El `Ejemplo` completo del spec sigue siendo obligatorio (nunca se omite) y debe ser **autocontenido** (define sus propias variables, sin depender de que se haya ejecutado el concepto anterior): alimenta la sección "ICN — código completo" del Solucionario y sigue siendo la única fuente del PPT, que no cambia. Excepción: si Diego pide explícitamente el formato antiguo (ejemplo completo + output) para una clase puntual, se omiten ambos campos y el generador vuelve solo a ese comportamiento — sin preguntarlo por default en cada clase nueva.
...
Esta pregunta es la contraparte de **cierre**; `Control.ipynb` (y su `Ejercitación.ipynb` hermana) además **abren** con la actitud — ver "Actitud y propósito abren ambos notebooks del día" en "Workflow: lunes estándar" más abajo.

**Reglas:**
- **La actitud la elige Diego**, igual que ya hace para la clase regular (regla 9). Para un `Control.ipynb` de lunes estándar, es la misma actitud definida en el Spec de esa clase. Para una `Evaluación.ipynb` sumativa (que abarca varias clases), Diego elige la actitud representativa de la unidad — no se asume ni se promedia automáticamente.
- **La pregunta de cierre la elige Diego al momento de generar cada Control/Evaluación.** El generador (script o skill que produzca `Control.ipynb` o `Evaluación.ipynb`) debe **preguntarle explícitamente** cuál de las preguntas del banco usar (por número o por familia) — nunca asumir una por default ni repetir siempre la misma.
- **Sin nota, nunca.** Va en el notebook de estudiante como una celda de texto abierta (mismo patrón que "Mis respuestas" — excepción de la regla 19), nunca en el Solucionario, y no se corrige ni se puntúa.
- **Ninguna pregunta del banco se puede responder en formato binario (sí/no).** Todas exigen narrar un momento concreto.
- **Rotar entre familias** en vez de repetir la misma pregunta o familia dos veces seguidas — mismo problema que ya se diagnosticó en trigonometría con la pregunta única de propósito.

**Banco de preguntas** (`{ACTITUD}` y `{VERBO_ACTITUD}` se completan con la actitud elegida por Diego para esa clase/unidad):

| # | Familia | Pregunta |
|---|---|---|
| 1 | Evidencia directa | "En esta evaluación, cuenta en qué ítem o ejercicio tuviste que aplicar {ACTITUD} y cómo se vio en lo que hiciste." |
| 2 | Evidencia + consecuencia | "Cuenta un momento en que aplicaste {ACTITUD} durante la evaluación. ¿Qué habría pasado con tu resultado si no lo hacías justo ahí?" |
| 3 | Dificultad/tensión | "Cuenta un momento de la evaluación en que te costó sostener {ACTITUD}, y qué hiciste para lograrlo igual." |
| 4 | Autoevaluación con escala | "Del 1 al 5, ¿cuánto sostuviste {ACTITUD} hoy? Cuenta el momento concreto que más lo demuestra." |
| 5 | Contraste entre partes | "¿En qué parte de la evaluación te costó más sostener {ACTITUD} que en el resto? Cuenta por qué ahí y no en otra parte." |
| 6 | Transferencia (no-programación) | "Cuenta un momento fuera de la programación —en el colegio, en tu casa, en algún pasatiempo— donde te ha tocado {VERBO_ACTITUD} igual que hoy." |
| 7 | Proyección a la próxima evaluación | "La próxima vez que rindas algo así, cuenta qué harías distinto para aplicar mejor {ACTITUD}." |
| 8 | Ítem específico marcado por Diego | "En el Ítem {N} tuviste que decidir entre dos caminos posibles. Cuenta cómo se notó {ACTITUD} en la forma en que decidiste." |
| 9 | Vocacional — futuro | "Piensa en lo que quieres estudiar o hacer más adelante. Cuenta de qué forma {ACTITUD} te podría servir ahí." |
| 10 | Vocacional — futuro | "Piensa en el trabajo o proyecto que te imaginas a futuro. Describe una situación concreta ahí donde vas a necesitar {VERBO_ACTITUD}." |
| 11 | Vocacional — futuro | "Cuenta qué parte del camino que quieres trazar (estudios, trabajo, algo propio) se parece a tener que {VERBO_ACTITUD}." |
| 12 | Vocacional — futuro | "Si pudieras llevarte solo una actitud de esta evaluación al resto de tu vida, cuenta por qué te llevarías justo {ACTITUD}." |

Las 4 familias son: hoy-evidencia (1, 2, 3, 8), hoy-comparación (4, 5), puente fuera de la programación (6, 7), y vocacional-futuro (9-12). Preferir rotar entre familias antes que dentro de la misma.

## Workflow: lunes estándar (ejercitación + control)

Vigente desde el **lunes 2026-08-17** (primera aplicación: Clase 23). Reemplaza al formato de ayudantía suelta como default de los lunes de consolidación regular. El mecanismo completo y el calendario de qué lunes lo usan están en `clases/Plan Cierre 4to Medio y Continuidad 3ro - Agosto a Noviembre 2026.md` §4 — este bloque solo fija lo que el agente necesita para producir los artefactos.

**Las 3 fases de la sesión (80 min):** sorteo de parejas con `random.sample()` en vivo (~5 min) → **Fase 1: Ejercitación en parejas sorteadas** (~45 min) → **Fase 2: Control individual con nota** (~20 min, entrega por Classroom) → **Fase 3: revisión y reflexión a viva voz** (~10 min, comprensión 1-5 + propósito). El lunes estándar **nunca** trae contenido nuevo y **nunca** trae Ticket de Salida: el Control reemplaza al TdS como instrumento de medición de ese día.

**Se diseña al revés de como se dicta.** Primero el Control (fija el criterio de logro), después la Ejercitación (entrena exactamente eso). Cada ejercicio de la Fase 1 tiene un ítem hermano en la Fase 2: **misma habilidad, contexto y estructura distintos** — nunca el mismo ejercicio con otros números, porque las soluciones de la Fase 1 quedan visibles en su propio notebook (excepción de la regla 5).

**Contexto disfrazado, nunca la consigna desnuda (default desde Clase 23).** Ni el Control ni la Ejercitación piden nunca "programa un `while` que…". Cada ítem abre con un contexto **muy breve** — una o dos líneas, bastante menos que la narrativa de 3-4 líneas de la Práctica Independiente — que obliga al estudiante a decidir por su cuenta qué estructura corresponde. La Ejercitación puede ir algo más larga y algo más difícil que el Control, pero sin alejarse: es el entrenamiento del mismo tipo de tarea, no un salto de exigencia.

<!-- why/evita/ej completos en la regla 16 del CLAUDE.md raíz (piloto en Clase 25, 2026-08-21) —
     acá solo se fija cómo aplica al formato de lunes estándar específicamente -->
**Ejercicios "0" de práctica directa — opcional en la Ejercitación, nunca en el Control (default desde 2026-08-21, ver regla 16 del CLAUDE.md raíz).** La Ejercitación puede abrir su serie de ejercicios con 1-2 ítems `Ejercicio 0a`/`Ejercicio 0b` antes del Ejercicio 1: consigna técnica explícita sin contexto disfrazado y sin pistas, aplicando la definición pelada del contenido (ej. "Define una función llamada X que reciba... y devuelva... con `return`"). Cantidad a criterio de quien diseña, según cuántos matices distintos tiene el contenido del día. **El Control nunca los incluye** — sus 3 ítems siguen siendo, siempre, contexto disfrazado + consigna que obliga a decidir la estructura, nunca la definición pelada.

**Actitud y propósito abren ambos notebooks del día (default desde Clase 23, 2026-08-13).** `Ejercitación.ipynb` abre con el objetivo y una sección `## 💡 Propósito` — mismo estilo reflexivo que usa `disenar-clase` (regla 9), ligando la actitud del día a lo que se entrena hoy. `Control.ipynb` abre, justo después del título y antes de "Antes de partir", con una frase corta y directa en imperativo — no la cita reflexiva del propósito, sino algo como "**En este control debemos perseverar hasta el último ítem.**" seguido de una frase concreta de qué hacer si cuesta. Ambos textos viven en un bloque `actitud` compartido en la Propuesta (`nombre`, `proposito_md` para la Ejercitación, `frase_control_md` para el Control) — Diego los redacta junto con el resto de la Propuesta; no se derivan de una plantilla genérica por actitud.

**`Ejercitación.ipynb` nunca menciona la modalidad de trabajo (parejas sorteadas u otra), en ninguna de sus celdas** — ni en la intro (Objetivo/Propósito), ni en el encabezado de la serie de ejercicios ("Ahora en parejas..." queda prohibido), ni en ningún enunciado. Eso lo anuncia Diego en vivo, el día de la clase, nunca queda fijo de antemano en un material que se sube a Colab con anticipación — mismo criterio que ya rige para `Clase.ipynb`/`Ejercicios.ipynb` (ver `notebook-sin-modalidad-trabajo` en memoria). Detectado en Clase 23 el 2026-08-13: dos celdas distintas de la misma Ejercitación traían la mención por separado (la intro y el encabezado de la serie) — al revisar este punto en el generador, chequear el notebook completo, no solo la primera celda.

**Artefactos** (carpeta `clases/clase-NN-lunes-tema-breve/`, prefijo `Clase NN - Tema - [Tipo]`):

| Archivo | Para quién | Contenido |
|---|---|---|
| `... - Propuesta.json` | interno | Fuente de verdad única: ejercitación + control + rúbrica. Nunca editar los `.ipynb` a mano |
| `... - Ejercitación.ipynb` | estudiantes (Colab) | Guiado corto + opcionalmente 1-2 Ejercicios "0" de práctica directa + serie de ejercicios + sección final `## 🔓 Soluciones` con cada solución dentro de un `<details>` |
| `... - Control.ipynb` | estudiantes (Classroom) | 3 ítems, **todos de programación**, siempre con contexto disfrazado — nunca Ejercicios "0". Sin ninguna solución |
...
**Corrección:** la hace un agente sobre el código extraído programáticamente (mismo extractor de `revisar-evaluacion`), siguiendo el método ya usado en la Evaluación 2. **Los errores de corrección los reclaman los estudiantes**, igual que en la evaluación: Diego no audita entrega por entrega.
...
**Cierre de la revisión (tras corregir con la rúbrica parcelada):** los Colabs personalizados de devolución (uno por estudiante, mismo motor `tools/review_eval/colab_devolucion.py` que usan las evaluaciones sumativas) se mueven a la carpeta compartida `clases/EVALUACIONES-REVISADAS/CONTROLES/<Clase NN - Nombre del control (contenido breve) - fecha AAAA-MM-DD>/` — esa es la carpeta que Diego comparte tal cual con el curso, así que ahí nunca van resúmenes/Excel agregados (`Controles - Trazabilidad.xlsx` incluido), solo el cuaderno individual de cada estudiante. Está en `.gitignore`.
...
**Ni la Evaluación ni el Simulacro llevan Ejercicios "0" de práctica directa (ver regla 16 del CLAUDE.md raíz).** Ese prefijo de calentamiento es exclusivo de instrumentos de práctica sin nota (Práctica Independiente regular, Ejercitación de lunes estándar) — acá el objetivo es medir en el mismo formato del instrumento real (contexto disfrazado en la Evaluación, ítems espejo de la Evaluación en el Simulacro), no aplicar la definición pelada.
...
**Cierre de actitud:** antes de generar `Evaluación.ipynb`, preguntar a Diego (a) qué actitud representa esta evaluación/unidad (puede abarcar varias clases — Diego elige, nunca se asume ni se promedia) y (b) qué pregunta del banco de "Cierre de actitud en Control y Evaluación" quiere usar. Esa pregunta se agrega en `Evaluación.ipynb` como celda final abierta, sin nota, nunca en ninguno de los dos Solucionarios.
...
5. **Colab personalizado de devolución** — motor en `tools/review_eval/colab_devolucion.py`; sirve para auditar y después se entrega al estudiante. Reemplaza al Excel de feedback como formato principal. **Al cierre de esta fase, los `.ipynb` se mueven a la carpeta compartida `clases/EVALUACIONES-REVISADAS/PRUEBAS/<carpeta detallada>/`** (mismo destino para Controles en `CONTROLES/`, ver "Workflow: lunes estándar") — es la carpeta que Diego comparte tal cual con el curso, así que ahí nunca van resúmenes/Excel agregados, solo el cuaderno individual de cada estudiante. Está en `.gitignore` (mismo criterio de privacidad que `revision/`).
6. **Generar resumen final** — `generar_resumen_final.py` produce md, csv, xlsx (excluye ausentes y excluidos) — queda en `revision/`, nunca en `EVALUACIONES-REVISADAS/`
