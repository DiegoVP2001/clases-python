...
**Piloto: ICN con código mínimo + instrucción (Clase 33, 2026-09-08 — demo puntual, NO default).** Cada concepto del ICN admite dos campos opcionales adicionales en el spec: `- Instrucción:` (una línea, imperativa, nombrando la sintaxis nueva) y `- Punto de partida:` (bloque ` ```python ` con solo el código previo a la sintaxis nueva, ej. la variable inicial). Si un concepto trae ambos, `Clase.ipynb` usa el "Punto de partida" en la celda de código (en vez del "Ejemplo" completo) y agrega, tras el blockquote de la Idea clave, la línea `✍️ **Escríbelo tú:** <instrucción>` — el estudiante escribe él mismo la sintaxis nueva encima de esa celda incompleta. El "Ejemplo" completo sigue siendo obligatorio en el spec (no se puede omitir): alimenta la nueva sección `## 2️⃣ Introducción al Contenido Nuevo — código completo` del Solucionario y sigue siendo la fuente del PPT. Origen: en Clase 32, dictando en vivo, Diego borró a mano el cuerpo de un ejemplo del ICN para que el curso completara la sintaxis nueva — este piloto lo deja disponible desde el spec en vez de editar el `.ipynb` a mano. Sin estos dos campos, el comportamiento es exactamente el de siempre (verificado regenerando Clase 32 y comparando celda por celda contra el `.ipynb` existente: 0 diferencias). Pendiente decisión de Diego sobre generalizarlo (regla en `CLAUDE.md` + defaults en `disenar-clase`/`generar-colab-clase`/`generar-ppt-clase`) tras ver el resultado en Clase 33.
