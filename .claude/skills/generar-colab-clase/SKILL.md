...
| 1 | Markdown | Objetivo y contenidos previos/nuevos (propósito y "¿Para qué sirve?" van en celda aparte — ver regla abajo) |
| 1b | Markdown | "💡 Propósito" + "🔎 ¿Para qué sirve?" (default desde Clase 33 — ver regla abajo) |
...
| 5+ | Code | Celdas del ICN — por default (ver regla abajo) solo el "Punto de partida" de cada concepto, incompleto a propósito |
| n-1 | Markdown | Tabla-resumen de cierre del ICN ("qué debe quedar claro en tu cuaderno") — solo si 2+ conceptos traen "Resumen tabla" en el spec, ver regla abajo |
...
**Tabla-resumen de cierre del ICN (default desde 2026-09-02, piloto Clase 29 — ver regla 23 del CLAUDE.md raíz y "Convención de Resumen tabla" en `disenar-clase/SKILL.md`).** Justo antes de "Errores típicos a evitar", si 2 o más conceptos del ICN traen el campo `- Resumen tabla:` en el spec, el generador arma sola una tabla `Concepto | Ejemplo` que condensa todos los conceptos en una sola vista comparativa. No requiere nada manual en el spec más que rellenar ese campo por concepto — la celda markdown se arma en `generar_resumen_icn_markdown()` de `crear_colab.py`. Con 0 o 1 concepto con el campo, no se genera (no aporta sobre la Idea clave que ya tiene ese concepto).

**ICN con código mínimo + instrucción — "Escríbelo tú" (default desde 2026-09-24, ver regla 25 del CLAUDE.md raíz; piloteado en Clase 33, 2026-09-08, generalizado tras corregir Clase 35).** Cada concepto del ICN trae, además de `Definición`/`Ejemplo`/`Idea clave`/`Resumen tabla`, los campos `- Instrucción:` (una línea, imperativa, nombrando la sintaxis nueva) y `- Punto de partida:` (bloque ` ```python ` con solo el código previo a la sintaxis nueva, ej. la variable inicial). Con ambos presentes, `Clase.ipynb` usa el "Punto de partida" en la celda de código (en vez del "Ejemplo" completo) y agrega, tras el blockquote de la Idea clave, la línea `✍️ **Escríbelo tú:** <instrucción>` — el estudiante escribe él mismo la sintaxis nueva encima de esa celda incompleta. El "Ejemplo" completo sigue siendo obligatorio en el spec (no se puede omitir) y debe ser autocontenido (define sus propias variables, sin depender de un concepto anterior): alimenta la sección `## 2️⃣ Introducción al Contenido Nuevo — código completo` del Solucionario y sigue siendo la fuente del PPT, que no cambia. Si un spec puntual no trae estos dos campos (excepción explícita de Diego, o specs anteriores a 2026-09-24), el comportamiento vuelve al formato antiguo sin romper nada (verificado regenerando Clase 32/33 y comparando celda por celda: 0 diferencias). Como las celdas del ICN quedan incompletas a propósito, en la práctica ya no producen outputs al ejecutarlas (ver "Después de generar" abajo).
...
1. **Ejecuta el notebook para verificar que el código corre sin errores**, antes de presentárselo a Diego:
   ...
   **Después de ejecutar, limpia los outputs que no correspondan:**
   ...
   ```
   La ejecución de verificación deja los outputs guardados dentro del `.ipynb`. **Solo el ICN conserva outputs** (el script no toca esa sección); el resto se limpia siempre. **Con el formato "Escríbelo tú" (default), en la práctica el ICN termina igual sin outputs**: las celdas quedan incompletas a propósito (solo el "Punto de partida", sin la línea que el estudiante escribe), así que al ejecutar no producen nada que conservar — no es que el script las limpie, es que no generan salida. (Si un spec puntual usa el formato antiguo sin "Punto de partida", el "Ejemplo" completo sí queda ejecutado con su output, como siempre — el estudiante lo lee junto a su resultado.) Un Haz Ahora cuyo trabajo es *ejecutar y observar* pierde el sentido si el resultado ya viene impreso, y una celda de verificación que trae su salida invita a leerla en vez de correrla.
...
**¿Para qué sirve? (default desde 2026-09-05, Clase 33 — ver regla 24 del CLAUDE.md raíz)**
- El contenido de la sección `## ¿Para qué sirve?` del spec se incluye en el notebook como `## 🔎 ¿Para qué sirve?` en blockquote `>`, inmediatamente después del `## 💡 Propósito` (misma celda markdown o la siguiente — antes del Haz Ahora en cualquier caso).
- Es contenido para estudiantes, igual que el Propósito. Si el spec no trae esta sección (specs anteriores a Clase 33), el generador simplemente omite el bloque — no es un error, es retrocompatibilidad.
...
**Ejercicios "0" de práctica directa (default desde 2026-08-21 — ver CLAUDE.md regla 16)**
Antes del Ejercicio 1, la Independiente puede traer 1-2 ejercicios `**Ejercicio 0a — [subtítulo]**` / `**Ejercicio 0b — [subtítulo]**`: mismo mecanismo `**El programa debe:**` + `**Celda de verificación:**` que el resto, pero **sin narrativa** (el texto antes de "El programa debe" queda vacío o con una frase de transición corta) y **sin pistas** — son consigna técnica explícita por diseño, nombrando el constructo sin disfraz (única excepción a la regla 8 de no nombrar operadores/constructos). `parsear_independiente()` en `crear_colab.py` acepta numeración alfanumérica (`\d+[a-z]?`) desde este cambio, así que "0a"/"0b" se parsean igual que cualquier otro número de ejercicio — no hace falta tocar el script para agregarlos a una clase nueva. **Nunca en Simulacro, Evaluación ni Control** (instrumentos con nota — ver CLAUDE.md regla 16).
