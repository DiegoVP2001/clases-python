...
CRITERIOS_CORRECCION_MD = (
    ...
    "Ramas repetidas, condiciones de más, nombres de variable raros o pasos "
    "innecesarios NO descuentan. Solo descuenta lo que produce un resultado "
    "equivocado. Antes de escribir cualquier comentario de descuento, revísalo: "
    "si estás señalando algo que no cambia lo que el programa entrega, sácalo. "
    "Por lo mismo, se califica el COMPORTAMIENTO y no la forma: una estructura "
    ...
    "**Da crédito parcial por componentes.** No califiques cada ítem como todo o "
    "nada: divídelo en las decisiones independientes que lo componen (leer el "
    "dato y su tipo, los valores exactos, los operadores, la estructura de "
    "ramas, el anidamiento entre bloques, los mensajes por camino) y puntúa cada "
    "una por separado. Un estudiante puede fallar una pieza y conservar el "
    "puntaje de las demás. Nunca descuentes el mismo error en dos componentes. "
    "En ítems de tipo \"arregla el bug\" el desglose va sobre el acto de "
    "corregir — Diagnóstico (¿tocó el elemento que fallaba?), Corrección (¿el "
    "cambio deja la lógica bien?) y Sin daños (¿el resto quedó intacto?) — con "
    "una regla de portazo: si entregó el fragmento tal como venía, sin tocar el "
    "error, el ítem completo es 0. Ver la skill `revisar-evaluacion` para el "
    "método completo.\n\n"
    "**Verifica ejecutando el código, no leyendo el output pegado en la celda.** "
    "Colab no re-ejecuta al editar, así que el resultado visible puede estar "
    "desactualizado. Si el estudiante cambió valores que venían fijos en el "
    "enunciado, repón los originales y califica la lógica que escribió.\n\n"
