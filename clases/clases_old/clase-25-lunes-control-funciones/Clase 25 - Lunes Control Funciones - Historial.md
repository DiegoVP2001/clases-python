...
| Felipe Aravena Cárdenas | 93/100 | 6,6 |
...
- Felipe Aravena Cárdenas (integrado más tarde el mismo día, ver abajo) fusionó en el Ítem 3 el cálculo del total bruto, los tres tramos y la resta dentro de una sola función con `return` — verificado por ejecución contra los umbrales de 10 y 20 kilos exactos, da el resultado correcto en todos los casos, pero no separa el descuento en su propio paso como pedía el enunciado.
...
| 20 Colabs `<Nombre> - Revisión Control.ipynb` | `clases/EVALUACIONES-REVISADAS/CONTROLES/Clase 25 - Control de Funciones - 2026-08-24/` (carpeta compartida con el curso) |
...
**Por ahora solo cubre Clase 25** (20 estudiantes, 1 control) — Clase 23 no está backfillada porque su script de corrección (`generar_devolucion_control23.py`) no escribió un `resultado_control23.json` en su momento. Es rápido de agregar si Diego lo pide (el script ya tiene toda la data en su dict `ESTUDIANTES`, solo falta el mismo bloque de exportación que se le agregó a Clase 25).

### 2026-08-25 (tarde) — Felipe Aravena Cárdenas: entrega integrada

Diego subió `FELIPE_ARAVENA_CONTROL.ipynb` (entrega vía pendrive) directo en `controles/`. Se copió a `controles_jupyters/Felipe Aravena - Clase 25 - Lunes Control Funciones - Control.ipynb`, reemplazando el archivo vacío que había quedado de la descarga de Classroom, y se agregó su alias a `extraer_control25.py`. El archivo original queda como respaldo en `controles/` a pedido explícito de Diego (no se borra).

Ítems 1 y 2 perfectos (25/25 y 30/30). Ítem 3: 38/45 con el mismo criterio benevolente de la tanda anterior — crédito completo donde el comportamiento es correcto (verificado por ejecución en los tres tramos, incluidos los bordes de 10 y 20 kilos), parcial solo en los dos componentes que miden explícitamente un paso separado (bruto aparte de la función, descuento guardado antes de restar) que su solución fusiona dentro de la función. **Total: 93/100, nota 6,6.**

Se regeneraron `codigo_extraido_control25.json` (20 estudiantes), `resultado_control25.json` (20 estudiantes) y los 20 Colabs de devolución (los 19 anteriores quedaron idénticos, verificado) + `Controles - Trazabilidad.xlsx`.
