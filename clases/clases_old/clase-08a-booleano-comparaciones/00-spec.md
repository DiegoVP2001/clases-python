...
**Estado:** Spec aprobada — 2026-05-21
...
- Definición: En Python existe un tipo de dato especial que solo puede tener dos valores: `True` (verdadero) o `False` (falso). Se escribe siempre con la primera letra en mayúscula. Puedes usar `type()` para confirmar que el tipo es `bool`.
- Ejemplo:
```python
tiene_stock = True
esta_agotado = False
...
```
- Idea clave: `True` y `False` son valores de datos, igual que los números.

**Concepto 2: Operadores de comparación**
- Definición: Son símbolos que comparan dos valores y devuelven `True` o `False` según si la comparación se cumple o no.
...
saldo_cuenta_rut = 45000
print(saldo_cuenta_rut >= precio_audifono)
```
...
- Definición: `bool()` convierte un número en su valor booleano. El número `0` se convierte en `False`; cualquier otro número se convierte en `True`.
- Ejemplo:
```python
ventas_realizadas = 0
print(bool(ventas_realizadas))
ventas_realizadas = 150
print(bool(ventas_realizadas))
```
- Idea clave: `0` siempre es `False`. Cualquier otro número es `True`.
