# Evaluación Final de Medio Semestre - Computación (4° Medio)
## Pensamiento Computacional y Programación

Este archivo contiene el cuestionario consolidado listo para ser cargado o copiado a la plataforma **Lirmi**, seguido de la pauta de corrección para el docente.
...
`nota1 = float(input("Nota 1: ")); nota2 = float(input("Nota 2: ")); promedio = (nota1 + nota2) / 2; print("Tu promedio es: " + promedio)`
`nota1 = float(input("Nota 1: ")); nota2 = float(input("Nota 2: ")); promedio = (nota1 + nota2) / 2; print("Tu promedio es:", promedio)`
`nota1 = input("Nota 1: "); nota2 = input("Nota 2: "); promedio = (int(nota1) + int(nota2)) / 2; print("Tu promedio es:", promedio)`
`nota1 = float(input("Nota 1: ")); nota2 = float(input("Nota 2: ")); promedio = nota1 + nota2 / 2; print("Tu promedio es:", promedio)`
Respuesta: `nota1 = float(input("Nota 1: ")); nota2 = float(input("Nota 2: ")); promedio = (nota1 + nota2) / 2; print("Tu promedio es:", promedio)`
OA: OA2 - IND-CS-2.2
Dificultad: medio
...
`cantidad = input("¿Cuántas entradas quieres? "); total = cantidad * 2500; print("El total a pagar es:", total)`
`cantidad = int(input("¿Cuántas entradas quieres? ")); total = cantidad * 2500; print("El total a pagar es:", total)`
`cantidad = float(input("¿Cuántas entradas quieres? ")); total = cantidad + 2500; print("El total a pagar es: " + total)`
`cantidad = int(input("¿Cuántas entradas quieres? ")); total = cantidad * "2500"; print("El total a pagar es:", total)`
Respuesta: `cantidad = int(input("¿Cuántas entradas quieres? ")); total = cantidad * 2500; print("El total a pagar es:", total)`
OA: OA2 - IND-CS-2.2
Dificultad: medio
...
Un estudiante intenta programar un aviso para saber si tiene exactamente nota 4.0 en su tarea de Computación, y escribe lo siguiente:
```python
nota = 4.0
if nota = 4.0:
...
## Pauta de Corrección (Docente) - Computación (4° Medio)

| Pregunta N° | Alternativa Correcta | OA / Indicador Asociado | Dificultad |
| :---: | :--- | :--- | :---: |
| **1** | bool | OA2 - `IND-CS-2.1` | facil |
| **2** | int | OA2 - `IND-CS-2.1` | facil |
| **3** | `promedio = float(input("Ingresa tu promedio: "))` | OA2 - `IND-CS-2.2` | facil |
| **4** | `edad = int(input("Ingresa tu edad: "))` | OA2 - `IND-CS-2.2` | facil |
| **5** | True | OAd - `IND-CS-d.1` | facil |
| **6** | Saldo insuficiente | OA3 - `IND-CS-3.1` | facil |
| **7** | `nota1 = float(input("Nota 1: ")); nota2 = float(input("Nota 2: ")); promedio = (nota1 + nota2) / 2; print("Tu promedio es:", promedio)` | OA2 - `IND-CS-2.2` | medio |
| **8** | `cantidad = int(input("¿Cuántas entradas quieres? ")); total = cantidad * 2500; print("El total a pagar es:", total)` | OA2 - `IND-CS-2.2` | medio |
| **9** | False | OAd - `IND-CS-d.1` | medio |
| **10** | False | OAd - `IND-CS-d.1` | medio |
| **11** | Requiere Acompañante Adulto | OA3 - `IND-CS-3.1` | medio |
| **12** | Total final: 4000 | OA3 - `IND-CS-3.1` | medio |
| **13** | Plan Estándar | OA1 - `IND-CS-1.1` | medio |
| **14** | Aprobado | OA1 - `IND-CS-1.1` | medio |
| **15** | Porque para comparar igualdad se debe usar el operador `==` en lugar de `=`. | OA3 - `IND-CS-3.2` | medio |
| **16** | Las sentencias dentro de los bloques `if` y `else` no tienen la indentación (sangría) obligatoria en Python. | OA3 - `IND-CS-3.2` | medio |
| **17** | True | OAd - `IND-CS-d.1` | dificil |
| **18** | Se imprimen: Oro, Plata y Bronce | OA3 - `IND-CS-3.1` | dificil |
| **19** | Precio final: 3200.0 | OA1 - `IND-CS-1.1` | dificil |
| **20** | Monto a pagar: 6300.0 | OA1 - `IND-CS-1.1` | dificil |
