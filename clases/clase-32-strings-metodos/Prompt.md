# Prompt de sesión — Clase N°32: Strings — métodos para modificar y separar texto
...
- **Contenido que se SACÓ de esta clase (no reintroducir aquí):**
  - Búsqueda de texto (`in`, `find()`) → se movió a la Clase N°33 nueva (`clase-33-strings-busqueda`), porque es una habilidad distinta (consultar vs. modificar).
  ...
  - `find()` (aislado) — se evaluó sacarlo por falta de aplicación real sin `in`, pero terminó yéndose junto con `in` a la Clase 33 (ahí sí tiene sentido, como par de búsqueda).
...
> Retomemos el diseño de la Clase 32 (Strings — métodos para modificar y separar texto). Ya acordamos actitud (Orden), contexto (Los Mellis como escenario único de toda la clase) y el recorte de contenido (sin búsqueda in/find, que se movió a Clase 33; sin f-strings, que quedó pendiente sin clase) — ver la sección "Progreso acordado en sesión 2026-08-26" arriba. Actívate con `disenar-clase`, ajusta el objetivo/propósito (el borrador anterior mencionaba f-strings, ya no aplica) y sigue desde el Paso 3 hacia la estructura de 5 pasos.

## Contexto acordado (planificación 2026-08-20, renumerada 2026-08-26 dos veces)

- Tercera de las clases de Strings dictadas en la misma semana/quincena: N°28 (indexing/slicing, lunes 31-ago) → N°29 (recorrido con `for`, martes 1-sep) → **N°32 (esta, jueves 3-sep)** → N°31 (lunes estándar, control con foco solo en N°28 y N°29 — esta clase queda fuera, 7-sep) → N°33 (búsqueda de texto, martes 8-sep). Nota: esta renumeración (2026-09-03) hace que el N° del control (31) sea menor que el de esta clase (32) pese a dictarse después — el N° identifica el artefacto, no el orden estricto de dictado (mismo criterio ya usado antes, ver `Historial-Curricular.md`).
...
- **Segunda renumeración 2026-08-26 (esta sesión):** el alcance de esta clase se recortó de "métodos y f-strings" a solo "métodos para modificar y separar texto" (ver "Progreso acordado" arriba). Carpeta renombrada de `clase-30-strings-avanzado` a `clase-30-strings-metodos`. El N° real (30) no cambió. Ver `Historial-Curricular.md` para el detalle completo de ambas renumeraciones. Renumerada de nuevo el 2026-09-03: pasa de N°30 a **N°32** (carpeta `clase-30-strings-metodos` → `clase-32-strings-metodos`) — ver nota de renumeración 2026-09-03 en `Historial-Curricular.md`.
