...
**Pendiente, no generado todavía:** `Solucionario Estudiantes.ipynb` — la función ya existe en `generar_evaluacion.py` (`build_solucionario_estudiantes_notebook`) pero no se invoca por defecto, según la regla del proyecto de que ese archivo se genera solo cuando Diego lo pide explícitamente.
